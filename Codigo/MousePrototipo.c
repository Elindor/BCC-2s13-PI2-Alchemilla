...
//inits (Y U NO funciona em uma funcao separada???)
   ...
   background = al_load_bitmap("al_proj/the game.png");
   if(!background){
       fprintf(stderr, "Bitmap nao foi criado.\n");
       return -1;
   }

   botaoIntro = al_load_bitmap("al_proj/botao.png");
   if(!botaoIntro){
   ...
   while(!sair){
       while(!al_is_event_queue_empty(filaEventos)){
           ...
           al_wait_for_event(filaEventos, &evento);
            ...
            //Define área em que quando o mouse estiver nela, o botão mudará de imagem
           if(evento.type == ALLEGRO_EVENT_MOUSE_AXES){
            ...
            //Define área em que quando o botão do mouse for pressionado e solto, o botão ativará intro
